...
### Bẫy Đọc Input Nhiều Dòng (Khoảng trống hoặc xuống dòng)
Hệ thống PTIT thường mang test case của C++ sang, nơi dữ liệu bị ngắt dòng lung tung (xuống dòng liên tục). Lệnh `input().split()` của Python **chỉ đọc được 1 dòng duy nhất**, dẫn đến thiếu dữ liệu.
Nếu đề bài có câu *"Các số phân cách bởi khoảng trống HOẶC XUỐNG DÒNG"*, tuyệt đối phải dùng vòng lặp `while` để bòn vét cho đủ quân số:
```python
n = int(input())  # Đã biết trước cần đọc đủ n số
a = []
while len(a) < n:  # Chừng nào mảng chưa gom đủ n quân số
    a.extend(map(int, input().split()))  # Tiếp tục đọc dòng tiếp theo và nhét vào đuôi mảng
```
